...
func NewExchange(name string, keys []string, connectionSettings ConnSettings) (*ExchangeMiddleware, error) {
...
func (em *ExchangeMiddleware) StartConsuming(callbackFunc func(msg Message, ack func(), nack func())) (err error) {
	...
	for {
		...
		for d := range msgs {
			copy := d
			callbackFunc(Message{Body: string(copy.Body)}, func() { copy.Ack(false) }, func() { copy.Nack(false, false) })
		}
...
func (em *ExchangeMiddleware) tryPublish(key string, msg Message, retries int) (err error) {
	_, err = retryWithBackoff(
		retries,
		func() (struct{}, error) {
			em.stateMutex.Lock()
			...
			case confirmation, ok := <-em.publishAck:
				if !ok {
					return placeHolder, ErrMessageMiddlewareDisconnected
				}

				if !confirmation.Ack {
					return placeHolder, ErrMessageMiddlewareMessage
				}
...
func (em *ExchangeMiddleware) Send(msg Message) (err error) {
	for _, key := range em.keys {
