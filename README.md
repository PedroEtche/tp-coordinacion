...
*Fig. : Diagragrama de Robustez de la solucion planteada*

Los cambios introducidos son los siguientes:

- Para soportar múltiples clientes, se asigna un UUID a cada uno, generado en el message handler. Adicionalmente, cada cliente numera de forma incremental las queries que emite, lo que permite su identificación y seguimiento.
- Para escalar la etapa de Sum, se reutiliza la misma cola para lograr balanceo de carga entre múltiples instancias. Cada vez que se procesa una query, su identificador es enviado al nodo Join. Cuando un nodo Sum recibe un mensaje de EOF correspondiente a un cliente, simplemente reenvía dicho evento al Join.
Dado que las queries de cada cliente están numeradas, el Join puede determinar cuántas queries fueron emitidas antes del EOF. En consecuencia, el Join espera a recibir la confirmación de procesamiento de todas las queries asociadas a ese cliente. Una vez completado este conjunto, emite una señal de safe to flush hacia todas las instancias de Sum.
Se identificó una mejora potencial en este punto: actualmente, cada nodo Sum envía los identificadores de queries procesadas de forma individual. Una alternativa más eficiente sería enviar estos identificadores en lotes, lo que reduciría la cantidad de mensajes y la latencia de red.
Sin embargo, los intentos de implementación introdujeron condiciones de carrera que no pude resolver en el tiempo disponible, por lo que esta optimización no fue incorporada en la versión final.
- Para soportar múltiples instancias de Aggregator, se mantiene el uso del exchange existente. La distribución se realiza mediante una función de hash sobre el identificador del cliente, seguida de una operación módulo con la cantidad de Aggregators disponibles. Esto garantiza una asignación determinística por cliente.
También se identificó una posible mejora: permitir que las queries de un mismo cliente se distribuyan entre múltiples Aggregators, y delegar en el Join la fusión de resultados parciales. Este enfoque permitiría una mayor paralelización.
Se avanzó en una implementación preliminar de esta estrategia, pero nuevamente surgieron condiciones de carrera cuya resolución excedía el tiempo disponible. Una posible solución futura sería adoptar un esquema similar al de Sum (uso de queues en lugar de exchange) y centralizar la coordinación en el Join.

En su estado actual, el sistema escala adecuadamente en función de la cantidad de clientes, ya que la carga se distribuye de manera uniforme entre las instancias de Sum. En cuanto a los Aggregators, si bien la distribución no es completamente uniforme, se observa una mejora significativa respecto a una configuración con una única instancia, especialmente cuando la carga computacional aumenta.

Las optimizaciones identificadas permitirían una mejora adicional en la escalabilidad y eficiencia, pero no fueron integradas debido a limitaciones de tiempo y a la aparición de problemas de concurrencia no resueltos. Esto no afecta la correctitud del sistema actual, pero sí su potencial de rendimiento máximo.